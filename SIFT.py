...
def generateBaseImage(image, sigma, assumed_blur):
    ...
    # max because if the value is zero or negative, the sqrt will return nan
    sigma_diff = sqrt(max((sigma ** 2) - ((2 * assumed_blur) ** 2), 0.01)) # sqrt to convert from variance to std deviation
...
def computeKeypointsWithOrientations(keypoint, octave_index, gaussian_image, radius_factor=3, num_bins=36, peak_ratio=0.8, scale_factor=1.5):
    """Compute orientations for each keypoint
     the actual value we place in that bin is that pixel’s gradient magnitude
    ...
    for n in range(num_bins):
        # derivation here https://theailearner.com/2019/05/06/gaussian-blurring/
        smooth_histogram[n] = (6 * raw_histogram[n] + 4 * (raw_histogram[n - 1] + raw_histogram[(n + 1) % num_bins]) + raw_histogram[n - 2] + raw_histogram[(n + 2) % num_bins]) / 16.
    orientation_max = max(smooth_histogram)
    orientation_peaks = where(logical_and(smooth_histogram > roll(smooth_histogram, 1), smooth_histogram > roll(smooth_histogram, -1)))[0]
    for peak_index in orientation_peaks:
        peak_value = smooth_histogram[peak_index]
        # derivation here https://ccrma.stanford.edu/~jos/sasp/Quadratic_Interpolation_Spectral_Peaks.html
        if peak_value >= peak_ratio * orientation_max:
            # Quadratic peak interpolation
            # The interpolation update derivation equation (6.30) in https://ccrma.stanford.edu/~jos/sasp/Quadratic_Interpolation_Spectral_Peaks.html
            ...
            # create keypoint for each peak .. identical in everything except orientation
            # for stability
...
def removeDuplicateKeypoints(keypoints):
    ...
    same as OpenCV : https://github.com/opencv/opencv/blob/master/modules/features2d/src/keypoint.cpp
    """
    if len(keypoints) < 2:
        return keypoints
...
def convertKeypointsToInputImageSize(keypoints):
    """Convert keypoint point, size, and octave from base image to input image size
        half the attributes
    """
    converted_keypoints = []
...
def generateDescriptors(keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3, descriptor_max_value=0.2):
    ...
    for keypoint in keypoints:
        ...
        # create histogram but rotate it to match the keypoint orientation
